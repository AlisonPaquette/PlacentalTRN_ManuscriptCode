...
load("/local/users/apaquett/PlacentalTRN/IntermediateData/RobustModelTFList.RData")
...
RobustTRN_Cor<- foreach(j=1:length(names(RobustModel_TFList)), .combine=rbind,.errorhandling='remove') %dopar% {
  tempMatrix = Corr_Results(Exprs_all,names(RobustModel_TFList)[j],RobustModel_TFList[[j]]) #calling a function
...
save(RobustTRN_Cor,file="/users/apaquett/PlacentalTRN/IntermediateData/FullDatasetRobustModel_CorrResults.RData")
