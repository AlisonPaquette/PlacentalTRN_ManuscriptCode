...
#Remove those TFs not defined by TF DB
  rownames(tbl.model)<-tbl.model$gene
  load("/users/apaquett/HumanDBTFs.RData")
  TFs<-intersect(tbl.model$gene,HumanTFDB_TFs)
  tbl.model<-tbl.model[TFs,]
...
save(tbl.models, file=file.path(target.dir, "GSM_Placenta11212019_ForHoldout.RData"))
